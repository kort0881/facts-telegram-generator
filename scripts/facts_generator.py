...
# -------------------------
PROMPT = """
... (оставляю без изменений, как у тебя выше)
"""


# -------------------------
# BANNED PHRASES / TITLE / STOP_WORDS / NORMALIZATION / SIMILARITY
# -------------------------
# (всё как в твоём последнем варианте, не меняю ради краткости)
# Скопируй эти секции из своего файла 1:1 — они корректны.


# -------------------------
# has_strict_fact_block (пока не используем в main)
# -------------------------
...
# -------------------------
# send_telegram (DRY-RUN) / pick_sources / mark_used
# -------------------------
# (оставляю без изменений, как в твоём последнем файле)
...
def main() -> None:
    ...
    for url in candidates:
        ...
        try:
            ...
            # ВНИМАНИЕ: строгую проверку первого блока временно отключаем
